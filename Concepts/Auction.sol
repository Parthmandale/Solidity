...
contract Bidding {
    ...
    struct Item {
        ...
        uint highestBid;
                                         ...
                                         // inside items all the key and values are stored of Item struct

    mapping(uint => Item) public items;  // a button named with item will be created which will tell the give all the info of stored in Item struct by just puting its uint id
    ...
    function createItem(string memory _name, string memory _description) public {
        itemCount++;
        items[itemCount] = Item(_name, _description, payable(msg.sender), 0);      // 1st item with = 
     }
